...
public class InputHandler
{
    ...
    public void Print(string data)
    {
        ...
        if (!string.IsNullOrEmpty(data))
        {
            ...
            if (PreviousCellEndsWithZeroWidthJoiner() && TryAppendToPreviousCell(data, codePoint))
            {
                return;
            }
        }
    ...
    private bool TryAppendToPreviousCell(string data, int codePoint)
    {
        ...
        int calculatedWidth = GetStringCellWidth(newContent);
        int newWidth = codePoint == ZeroWidthJoiner || calculatedWidth <= 0
            ? prevCell.Width
            : Math.Clamp(calculatedWidth, 1, 2);
    ...
    private bool PreviousCellEndsWithZeroWidthJoiner()
    {
        var line = _buffer.Lines[_buffer.Y + _buffer.YBase];
        if (line == null)
            return false;
        ...
        return prevCell.Content.EndsWith(char.ConvertFromUtf32(ZeroWidthJoiner), StringComparison.Ordinal);
    }
    ...
    private void CharAttributes(Params parameters)
    {
        ...
        if (parameters.RawText.Contains(':', StringComparison.Ordinal))
        {
            CharAttributes(parameters.RawText);
            return;
        }
    ...
    private void CharAttributes(string rawText)
    {
        ...
        string[] parameters = rawText.Split(';');
        for (int i = 0; i < parameters.Length; i++)
        {
            string parameter = parameters[i];
            if (parameter.Contains(':', StringComparison.Ordinal))
            {
                ApplyColonCharAttribute(parameter);
                continue;
            }

            int param = ParseSgrInt(parameter, 0);
            switch (param)
            {
                case 38:
                    i = HandleExtendedColor(parameters, i, true);
    ...
    private void ApplyColonCharAttribute(string parameter)
    {
        string[] parts = parameter.Split(':');
        int param = ParseSgrInt(parts.Length > 0 ? parts[0] : string.Empty, 0);

        switch (param)
        {
            case 38:
                TryApplyColonExtendedColor(parts, 1, true);
                break;
            case 48:
                TryApplyColonExtendedColor(parts, 1, false);
                break;
            case 58:
    ...
    private int HandleExtendedColor(Params parameters, int index, bool isForeground)
    {
        ...
        {
            SetRgbColor(
                parameters.GetParam(index + 2, 0),
                parameters.GetParam(index + 3, 0),
        ...
        else if (colorType == 5 && index + 2 < parameters.Length) // 256 color
        {
            ...
            SetPaletteColor(color, isForeground);
    ...
    private int HandleExtendedColor(string[] parameters, int index, bool isForeground)
    {
        if (index + 1 >= parameters.Length)
            return index;

        string colorTypeText = parameters[index + 1];
        if (colorTypeText.Contains(':', StringComparison.Ordinal))
        {
            TryApplyColonExtendedColor(colorTypeText.Split(':'), 0, isForeground);
            return index + 1;
        }

        int colorType = ParseSgrInt(colorTypeText, 0);

        if (colorType == 2 && index + 4 < parameters.Length) // RGB
        {
            SetRgbColor(
                ParseSgrInt(parameters[index + 2], 0),
                ParseSgrInt(parameters[index + 3], 0),
                ParseSgrInt(parameters[index + 4], 0),
                isForeground);
        ...
        {
            SetPaletteColor(ParseSgrInt(parameters[index + 2], 0), isForeground);
    ...
    private bool TryApplyColonExtendedColor(string[] parts, int colorTypeIndex, bool isForeground)
    {
        if (colorTypeIndex < 0 || colorTypeIndex >= parts.Length)
            return false;

        int colorType = ParseSgrInt(parts[colorTypeIndex], -1);
        if (colorType == 2)
        {
            List<int> values = ParseColonColorValues(parts, colorTypeIndex + 1);
            if (values.Count < 3)
                return false;

            SetRgbColor(values[^3], values[^2], values[^1], isForeground);
            return true;
        }
        else if (colorType == 5)
        {
            ...
            SetPaletteColor(values[0], isForeground);
            return true;
        }
    ...
    private static List<int> ParseColonColorValues(string[] parts, int startIndex)
    {
        List<int> values = new List<int>();
        for (int i = startIndex; i < parts.Length; i++)
        {
            if (TryParseSgrInt(parts[i], out int value))
            {
                values.Add(value);
            }
        }
    ...
    private void SetRgbColor(int r, int g, int b, bool isForeground)
    {
    ...
    private void SetPaletteColor(int color, bool isForeground)
    {
    ...
    private static int ParseSgrInt(string value, int defaultValue)
    {
        return TryParseSgrInt(value, out int parsed) ? parsed : defaultValue;
    }

    private static bool TryParseSgrInt(string value, out int parsed)
    {
